    ...
    pub fn evaluate(&self) -> i32 {
        ...
        for color in (Color::White as usize)..=(Color::Black as usize) {
            for piece in (Piece::Pawn as usize)..=(Piece::King as usize) {
                let mut bitboard = self.board.get_piece_of_color(piece.try_into().unwrap(), color.try_into().unwrap());
                while bitboard > 0 {
                    // material score
                    ...
                    match (c, p) {
                        ...
                        (Color::Black, Piece::Pawn) => score -= PAWN_SCORE[MIRROR_SCORE[s as usize] as usize],
                        (Color::Black, Piece::Rook) => score -= ROOK_SCORE[MIRROR_SCORE[s as usize] as usize],
                        (Color::Black, Piece::Knight) => score -= KNIGHT_SCORE[MIRROR_SCORE[s as usize] as usize],
                        (Color::Black, Piece::Bishop) => score -= BISHOP_SCORE[MIRROR_SCORE[s as usize] as usize],
                        (Color::Black, Piece::King) => score -= KING_SCORE[MIRROR_SCORE[s as usize] as usize],
